...
def run_condition(language, language_name, label_lang, label_condition):
    ...
    # LaBSE does not use prefixes
    label_descriptors = label_descriptors_raw
    ...
    # Print summary
    print(f"\nResults for LaBSE + BGE-reranker-v2-m3 | {language_name} | {label_condition}")
    for k in k_values:
    ...
    # Save results
    results_to_save = {
        'model': 'LaBSE + bge-reranker-v2-m3',
        'dataset': 'MultiEURLEX',
    ...
    filename = f'{prefix}results_labse_reranked_{language}_{label_condition.replace(" ", "_").lower()}.json'
